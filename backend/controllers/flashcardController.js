...
export const getFlashcards = async (req, res, next) => {
    try {
        ...
        const formattedFlashcards = flashcards.map((set) => {
            const reviewedCards = set.cards.filter(
            ...
            const progress =
                set.cards.length > 0
                    ? Math.round(
                          (reviewedCards / set.cards.length) * 100
                      )
                    : 0;
...
export const getAllFlashcardSets = async (req, res, next) => {
    try {
        ...
        console.log("Flashcard Sets Found:", flashcardSets.length);
        ...
        const formattedFlashcardSets = flashcardSets.map((set) => {
            ...
            const progress =
                set.cards.length > 0
                    ? Math.round(
                          (reviewedCards / set.cards.length) *
                              100
                      )
                    : 0;
        ...
        res.status(200).json({
            success: true,
            count: formattedFlashcardSets.length,
            data: formattedFlashcardSets,
        });
